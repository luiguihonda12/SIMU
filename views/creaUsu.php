<div class="registration-container">
    ...
    <form method="post" action="index.php?pg=creaUsu" novalidate>
        ...
        <div class="input-group"><label for="id_rol">Permiso</label><select id="id_rol" name="id_rol"><option value="3">Solo lectura</option><option value="2">Operador: crear y consultar</option><option value="1">Administrador: control total</option></select></div>
...
<?php $usuarios = $db ? $db->query('SELECT u.id_usuario, u.nombre, u.correo, u.id_rol, r.nombre_del_rol FROM usuario u INNER JOIN rol r ON r.id_rol = u.id_rol ORDER BY u.id_usuario DESC')->fetchAll() : []; ?>
<section class="module-card users-list-card">
    ...
    <div class="table-responsive"><table class="table align-middle mb-0"><thead><tr><th>Nombre</th><th>Correo</th><th>Rol</th><th></th></tr></thead><tbody><?php foreach ($usuarios as $usuario): ?><tr><td><?= e($usuario['nombre']) ?></td><td><?= e($usuario['correo']) ?></td><td><?= e(role_label((int) $usuario['id_rol'])) ?></td><td><?php if ((int) $usuario['id_usuario'] !== (int) current_user()['id_usuario']): ?><form method="post" action="index.php?pg=creaUsu" onsubmit="return confirm('¿Eliminar este usuario?');"><?= csrf_field() ?><input type="hidden" name="action" value="eliminar_usuario"><input type="hidden" name="id_usuario" value="<?= (int) $usuario['id_usuario'] ?>"><button class="btn btn-sm btn-outline-danger" type="submit">Eliminar</button></form><?php endif; ?></td></tr><?php endforeach; ?></tbody></table></div>
</section>
